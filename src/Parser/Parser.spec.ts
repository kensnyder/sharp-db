...
describe('Parser', () => {
  describe('column handler', () => {
    it('should recognize asterisk', () => {
      const query = Select.parse(adapter, 'SELECT * FROM mytable');
  ...
  describe('subquery handler', () => {
    it('should handle column subqueries', () => {
      ...
      const query = Select.parse(adapter, normalized);
  ...
  describe('JOIN handler', () => {
    ...
    joins.forEach(join => {
      it(`should handle ${join}`, () => {
        ...
        const query = Select.parse(adapter, sql);
  ...
  describe('conditions handler', () => {
    it('should parse WHERE 1 = 1', () => {
      const query = Select.parse(adapter, 'SELECT * FROM mytable WHERE 1 = 1');
      expect(query._wheres).toEqual(['1 = 1']);
    });
    it("should parse WHERE '1'", () => {
      const query = Select.parse(adapter, "SELECT * FROM mytable WHERE '1'");
      expect(query._wheres).toEqual(["'1'"]);
    });
    it('should parse WHERE true', () => {
      const query = Select.parse(adapter, 'SELECT * FROM mytable WHERE true');
  ...
  it('should parse ORDER BY', () => {
    const query = Select.parse(adapter, 'SELECT * FROM users ORDER BY id DESC');
    expect(query._orderBys).toEqual(['id DESC']);
  });
  describe('pagination', () => {
    it('should parse LIMIT with number', () => {
      const query = Select.parse(adapter, 'SELECT * FROM mytable LIMIT 1');
      expect(query._limit).toBe(1);
    });
    it('should parse LIMIT with placeholder', () => {
      const query = Select.parse(adapter, 'SELECT * FROM mytable LIMIT ?');
      expect(query._limit).toBe('?');
    });
    it('should parse LIMIT with named placeholder', () => {
      const query = Select.parse(adapter, 'SELECT * FROM mytable LIMIT :limit');
    ...
    it('should parse OFFSET with placeholder', () => {
      const query = Select.parse(
        adapter,
        'SELECT * FROM mytable LIMIT 5 OFFSET ?'
      );
    ...
    it('should parse OFFSET, LIMIT with numbers', () => {
      const query = Select.parse(adapter, 'SELECT * FROM mytable LIMIT 10, 5');
    ...
    it('should parse OFFSET, LIMIT with named placeholders', () => {
      const query = Select.parse(
        adapter,
        'SELECT * FROM mytable LIMIT :offset, :limit'
      );
      expect(query._limit).toBe(':limit');
      expect(query._offset).toBe(':offset');
    });
